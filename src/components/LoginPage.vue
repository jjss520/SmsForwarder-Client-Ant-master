      ...
      <a-space direction="vertical">
        <a-row>
          ...
          <a-col>
            <a-auto-complete
              v-model="serverUrl"
              :data-source="historyServer.map(item => item.serverUrl)"
              placeholder="请输入服务端地址"
              style="width: 250px;"
              allow-clear
              @select="onServerUrlSelect"
            >
              <template #prefix>
                <a-icon type="link" style="color: rgba(0,0,0,.25)" />
              </template>
            </a-auto-complete>
          </a-col>
        </a-row>
...
export default {
  ...
  methods: {
    ...
    onServerUrlSelect(url) {
      // 当用户选择历史服务端地址时，查找对应的密钥并填充
      const serverConfig = this.historyServer.find(item => item.serverUrl === url)
      if (serverConfig) {
        this.serverUrl = serverConfig.serverUrl
        this.secret = serverConfig.secret
      }
    }
  },
  watch: {
