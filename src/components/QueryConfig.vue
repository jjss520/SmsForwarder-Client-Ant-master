...
export default {
  ...
  methods: {
    ...
    push() {
      let timestamp = new Date().getTime();
      console.log('开始推送配置，当前json数据类型:', typeof this.json);
      console.log('当前json数据:', this.json);
      
      // 确保推送的数据是有效的JSON对象
      let pushData;
      try {
        if (typeof this.json === 'string') {
          pushData = JSON.parse(this.json);
        } else {
          pushData = this.json;
        }
        console.log('解析后的数据:', pushData);
        
        // 确保数据中包含版本号
        if (!pushData.version_code) {
          pushData.version_code = tools.getOrDefault(this.configQuery, 'version_code', -1);
          console.log('添加版本号:', pushData.version_code);
        }
      } catch (e) {
        console.error('解析JSON数据失败:', e);
        Vue.prototype.$message.error('推送失败: 配置数据格式错误');
        return;
      }
      ...
      this.$axios({
        ...
        console.log('推送配置响应:', res.data);
        if (res.data && res.data.code === 200) {
          Vue.prototype.$message.success("推送成功!");
        } else {
          Vue.prototype.$message.error('推送失败: ' + (res.data ? res.data.msg : '未知错误'));
        }
      }).catch((err) => {
        console.error('推送配置出错:', err);
        Vue.prototype.$message.error('推送失败: ' + (err.message || '网络错误'));
      });
    }
  },
  watch: {}
