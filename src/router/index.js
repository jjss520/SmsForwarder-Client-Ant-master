...
let router = new VueRouter({
    ...
    base: process.env.BASE_URL, routes: [{
        ...
        path: '/sendSms', name: 'sendSms', meta: {
            title: '发短信', icon: require("@/assets/logo.png"), auth: ENABLE_API_SMS_SEND
        }, // 懒加载模式
        ...
        path: '/location', name: 'location', meta: {
            title: '定位'
        }, // 懒加载模式
        component: () => import('@/components/LocationQuery.vue')
    }, {
...
router.beforeEach((to, from, next) => {
    ...
    // 始终显示固定的标题和图标
    document.title = "SmsForwarder远程控制器"
    // 始终使用logo.png作为图标
    tools.setIcon(require("@/assets/logo.png"))
    console.log(to)
